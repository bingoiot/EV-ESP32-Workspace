...
void app_main()
{
	...
	osLogI(1, "app_main boot partition		:type:%08x subtype:%08x run lable:%s addr:%08x size:%08x \r\n",
			pboot->type,pboot->subtype,pboot->label,pboot->address,pboot->size);
	osLogI(1, "app_main runnung partition	:type:%08x subtype:%08x run lable:%s addr:%08x size:%08x \r\n",
