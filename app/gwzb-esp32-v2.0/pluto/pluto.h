...
{
	option_default = 0x00,
	option_skip_scene = 0x01,
	option_skip_route  = 0x02,
	option_no_record = 0x03,
	option_return_send_state= 0x04,
}send_option_t;
...
extern void 		pluto_registe_led(led_blink_cb_t cb);
extern void 		pluto_led_blink(int num, int htime, int ltime);

extern uint8* 		pluto_get_local_key(uint8 keyID);
extern uint8* 		pluto_get_default_key(void);
extern uint8*		pluto_get_chip_id(void);
